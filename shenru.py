# SHENRU TOOLS PREMIUM v5.1 — Telegram Bot with Real API Creds
...
try:
    ...
    BOT_ENABLED = bool(BOT_TOKEN and TELEGRAM_API_ID and TELEGRAM_API_HASH)
except ImportError:
    BOT_TOKEN = ""
    TELEGRAM_API_ID = 0
    TELEGRAM_API_HASH = ""
...
# ── TELEGRAM BOT SPAMMER (USES REAL API CREDS FROM ENV) ────────────────────
async def bot_send_messages(target, message, count, mode):
    if not BOT_ENABLED:
        raise Exception("Bot not configured. Set BOT_TOKEN, TELEGRAM_API_ID, and TELEGRAM_API_HASH.")
    
    client = TelegramClient('bot_session', api_id=TELEGRAM_API_ID, api_hash=TELEGRAM_API_HASH)
...
def bot_spam_sync(target, message, count, mode):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        results, success = loop.run_until_complete(
            bot_send_messages(target, message, count, mode)
        )
        return results, success
    except Exception as e:
        raise Exception(f"Bot error: {str(e)}")
    finally:
        loop.close()
...
<section class="card full">
  <h2><span class="ico">🤖</span>TELEGRAM BOT SPAMMER + BOMBER</h2>
  <div style="font-size:9px;color:var(--mut);margin-bottom:8px">Bot sends messages to any user or group. No credentials needed.</div>
  
  <label>Target (@username or group invite link)</label>
  <input type="text" id="tg-target" placeholder="@username or https://t.me/joinchat/...">
...
async function sendTelegram(){
  ...
  if(!target) return toast('Enter a target (@username or group invite link)','warn');
  if(!message) return toast('Enter a message','warn');
  
  const btn = $('tg-btn'); setLD(btn, true);
  show('tg-result','⏳ Sending messages via Telegram bot...\nThis may take a moment.','warn');
  try{
    const d = await apiCall('/api/telegram', {
      target, message, count, mode: tgMode
    });
    show('tg-result', JSON.stringify(d, null, 2), 'ok');
    if(d.success) toast(`Sent ${d.sent}/${d.total} messages! 🤖`,'ok');
  } catch(e) {
...
def telegram_endpoint():
    try:
        ...
        if not BOT_ENABLED:
            return jsonify(error="Bot not configured. Set BOT_TOKEN, TELEGRAM_API_ID, and TELEGRAM_API_HASH."), 503
