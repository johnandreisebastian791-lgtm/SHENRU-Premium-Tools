# SHENRU TOOLS PREMIUM — Telegram Spammer + Bomber Edition
# Users enter their own Telegram credentials in the UI
...
# ── TELEGRAM SPAMMER + BOMBER (USER PROVIDES CREDENTIALS) ──────────────────
async def send_telegram_messages(target, message, count, mode, phone, api_id, api_hash):
    ...
    try:
        ...
        raise Exception(f"Target '{target}' not found. Use @username, phone number, or chat ID.")
...
async function sendTelegram(){
  ...
  try{
    ...
    show('tg-result', JSON.stringify(d, null, 2), d.error ? 'err' : 'ok');
    if(!d.error) toast('Telegram messages sent! ✈️','ok');
  } catch(e) {
...
def telegram_endpoint():
    ...
    try:
        ...
        return jsonify(total=len(results), success=success, mode=mode, results=results[:30])
    except Exception as e:
