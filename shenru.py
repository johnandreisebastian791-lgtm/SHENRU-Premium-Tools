# SHENRU TOOLS PREMIUM v4.0 — REAL BULK SMS + TELEGRAM FIXED
# SMS uses Twilio (real bulk), Telegram handles login code

from flask import Flask, request, jsonify, render_template_string, send_file, after_this_request
from functools import wraps
import os, re, json, time, uuid, urllib.parse, urllib.request, requests, asyncio, random
from concurrent.futures import ThreadPoolExecutor, as_completed
import google.generativeai as genai
from telethon import TelegramClient, errors

# ── TWILIO (OPTIONAL — FOR REAL BULK SMS) ──────────────────────────────────
try:
...
# ── TELEGRAM SPAMMER (FIXED — HANDLES LOGIN CODE) ─────────────────────────
async def send_telegram_messages(target, message, count, mode, phone, api_id, api_hash, code=None):
    ...
    except (errors.UsernameNotOccupiedError, ValueError) as e:
        await client.disconnect()
        raise Exception(f"Target '{target}' not found: {str(e)}")
    
    delay = 0.3 if mode == "bomb" else 1.0
    max_count = 200 if mode == "bomb" else 100
    results = []
    success_count = 0
    ...
    for i in range(min(count, max_count)):
        try:
            ...
            success_count += 1
            results.append({"attempt": i+1, "status": "sent", "ok": True})
        except Exception as e:
            results.append({"attempt": i+1, "status": str(e), "ok": False})
        await asyncio.sleep(delay)
    
    await client.disconnect()
    return results, success_count

def telegram_spam_sync(target, message, count, mode, phone, api_id, api_hash, code=None):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        results, success = loop.run_until_complete(
            send_telegram_messages(target, message, count, mode, phone, api_id, api_hash, code)
        )
        return results, success
    except Exception as e:
        error_msg = str(e).lower()
        if "code" in error_msg or "password" in error_msg or "login" in error_msg:
            raise Exception("LOGIN_CODE_REQUIRED: Enter the code sent to your Telegram app")
        raise Exception(f"Telegram error: {str(e)}")
    finally:
        loop.close()

# ── SMS BOMBER (REAL BULK — TWILIO) ────────────────────────────────────────
def sms_bomb(phone, count):
    ...
    # 1. Use Twilio if configured (REAL BULK SMS)
        ...
        for i in range(min(count, 500)):
            try:
                msg = client.messages.create(
                    body=f"Hello from Shenru {i+1}",
                    from_=TWILIO_PHONE,
                    to=phone
                )
                success_count += 1
                results.append({"attempt": i+1, "ok": True, "sid": msg.sid})
            except Exception as e:
                results.append({"attempt": i+1, "ok": False, "error": str(e)})
            time.sleep(0.2)
        return results, success_count
    
    # 2. Fallback: Free APIs (limited)
    apis = [
        {"url": "https://textbelt.com/text", "data": {"phone": phone, "message": "Hello", "key": "textbelt"}, "type": "post"},
        {"url": "https://api.textlocal.com/send", "data": {"apikey": "test", "numbers": phone, "message": "Hi"}, "type": "post"},
        {"url": "https://smsapi.free-mobile.fr/sendmsg", "params": {"user": "test", "pass": "test", "msg": "Test"}, "type": "get"},
    ]
    results = []
    success_count = 0
    for i in range(min(count, 30)):
        ...
        try:
            if api["type"] == "get":
                r = requests.get(api["url"], params=api.get("params", {}), timeout=8)
            else:
                r = requests.post(api["url"], data=api.get("data", {}), timeout=8)
            if r.status_code < 400:
                success_count += 1
                results.append({"attempt": i+1, "ok": True})
            else:
                results.append({"attempt": i+1, "ok": False})
        except:
            results.append({"attempt": i+1, "ok": False})
        time.sleep(0.5)
    return results, success_count
...
<title>SHENRU TOOLS — PREMIUM v4.0</title>
...
<section class="card full">
  <h2><span class="ico">✈️</span>TELEGRAM SPAMMER + BOMBER</h2>
  <div style="font-size:9px;color:var(--mut);margin-bottom:8px">Enter your own Telegram credentials. Messages sent from YOUR account.</div>
  
  <label>Your Telegram Phone Number</label>
  <input type="text" id="tg-phone" placeholder="+639123456789">
  
  <label>API ID</label>
  <input type="text" id="tg-api-id" placeholder="Get from my.telegram.org/apps">
  
  <label>API Hash</label>
  <input type="text" id="tg-api-hash" placeholder="Your API hash">
  
  <label>Target (@username, phone, or chat ID)</label>
  <input type="text" id="tg-target" placeholder="@username or 123456789">
  
  <label>Message</label>
  <input type="text" id="tg-message" placeholder="Your message here...">
  
  <label>Telegram Login Code (leave blank if already logged in)</label>
  <input type="text" id="tg-code" placeholder="Enter code from Telegram app (if prompted)">
  
  <div class="row">
    <div><label>Count</label>
      <input type="number" id="tg-count" value="10" min="1" max="200"></div>
    <div><label>Mode</label>
      <div class="type-row" style="margin-top:0">
        <div class="ttype on" id="tg-mode-spam" onclick="setTgMode('spam')">🐢 SPAM</div>
        <div class="ttype" id="tg-mode-bomb" onclick="setTgMode('bomb')">💣 BOMB</div>
      </div>
    </div>
  </div>
  
  <button class="btn" id="tg-btn" onclick="sendTelegram()">🚀 &nbsp;SEND MESSAGES</button>
  <div class="result" id="tg-result"></div>
</section>
...
<section class="card">
  ...
  <div style="font-size:9px;color:var(--mut);margin-bottom:8px">
    <span style="color:var(--gold)">⚡ REAL BULK SMS</span> — Uses Twilio (if configured) or Free APIs (limited)
  </div>
  <label>Phone Number</label>
  <input type="text" id="sms-phone" placeholder="09123456789 or +639123456789">
  <label>Bomb Count</label>
  <input type="number" id="sms-count" value="10" min="1" max="500">
...
<!-- ═══ AI CHAT ════════════════════════════════════════════════════════════ -->
<section class="card full">
  <h2><span class="ico">🤖</span>AI CHAT — Gemini 3.6 Flash</h2>
  <div class="cbox" id="cbox">
    <div class="bbl a"><div class="lbl">SHENRU AI</div>Wazzup! I'm your AI assistant powered by Gemini 3.6 Flash. Ask me anything 🔥</div>
  </div>
  <div class="chat-row">
...
async function sendTelegram(){
  ...
  const api_id = $('tg-api-id').value.trim();
  const api_hash = $('tg-api-hash').value.trim();
  const target = $('tg-target').value.trim();
  const message = $('tg-message').value.trim();
  const code = $('tg-code').value.trim() || null;
  const count = parseInt($('tg-count').value) || 10;
  
  if(!phone) return toast('Enter your phone number','warn');
  if(!api_id) return toast('Enter your API ID','warn');
  if(!api_hash) return toast('Enter your API Hash','warn');
  if(!target) return toast('Enter a target (@username, phone, or chat ID)','warn');
  if(!message) return toast('Enter a message','warn');
  
  const btn = $('tg-btn'); setLD(btn, true);
  show('tg-result','⏳ Sending messages via Telegram...\nThis may take a moment.','warn');
  try{
    const d = await apiCall('/api/telegram', {
      phone, api_id, api_hash, target, message, code, count, mode: tgMode
    });
    show('tg-result', JSON.stringify(d, null, 2), 'ok');
    if(d.success) toast(`Sent ${d.sent}/${d.total} messages! ✈️`,'ok');
  } catch(e) {
    if(e.message.includes('LOGIN_CODE_REQUIRED')) {
      toast('Please enter the Telegram login code sent to your app','warn');
      show('tg-result','⚠️ LOGIN CODE REQUIRED\n\nEnter the code from your Telegram app in the field above and try again.','err');
    } else {
      show('tg-result','❌ '+e.message,'err');
    }
  }
  setLD(btn, false);
}
...
async function sendSMS(){
  ...
  if(count > 500)return toast('Max 500 SMS per bomb','warn');
  const btn=$('sms-btn');setLD(btn,true);
  show('sms-result','⏳ Sending SMS bomb... (this may take a moment)','warn');
  try{
    const d=await apiCall('/api/sms',{phone,count});
    show('sms-result',JSON.stringify(d,null,2),'ok');
    if(d.success) toast(`Sent ${d.sent}/${d.total} SMS! 💥`,'ok');
...
# ── TELEGRAM SPAMMER (WITH LOGIN CODE HANDLING) ──────────────────────────
...
def telegram_endpoint():
    try:
        ...
        if d is None:
            return jsonify(error="Missing or empty JSON payload"), 400
        ...
        results, success = telegram_spam_sync(target, message, count, mode, phone, api_id, api_hash, code)
        return jsonify({
            "success": True,
            "total": len(results),
            "sent": success,
...
# ── SMS BOMBER (REAL BULK WITH TWILIO) ────────────────────────────────────
@app.post("/api/sms")
@require_key
def sms_endpoint():
    try:
        ...
        phone = d.get("phone", "").strip()
        count = max(1, min(int(d.get("count", 10)), 500))
        ...
        if TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN and TWILIO_PHONE:
            results, success = sms_bomb(phone, count)
            return jsonify({
                "success": True,
                "total": len(results),
                "sent": success,
                "provider": "Twilio (Bulk SMS)",
                "results": results[:20]
            })
        else:
            # Free APIs (limited)
            ...
            return jsonify({
                ...
                "total": len(results),
                "sent": success,
                "provider": "Free APIs (limited to 30 messages)",
                "results": results[:20],
                "warning": "For 500+ SMS, configure Twilio (TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE)"
            })
    except Exception as e:
        return jsonify(error=str(e)), 500
