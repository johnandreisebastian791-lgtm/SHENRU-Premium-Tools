# SHENRU TOOLS PREMIUM — single-file Flask panel
# Panel creds live server-side. Users just boost.
...
# ── YOUR SERVER-SIDE CONFIG (set these as env vars, never expose to users) ──
PANEL_URL        = os.environ.get("PANEL_URL", "")           # e.g. https://yourpanel.com
PANEL_KEY        = os.environ.get("PANEL_KEY", "")           # your panel API key
LIKES_SVC_ID     = os.environ.get("LIKES_SERVICE_ID", "")    # TikTok likes service ID
FOLLOWERS_SVC_ID = os.environ.get("FOLLOWERS_SERVICE_ID","") # TikTok followers service ID
ACCESS_KEYS      = [k.strip() for k in os.environ.get("ACCESS_KEYS","").split(",") if k.strip()]
ADMIN_KEY        = os.environ.get("ADMIN_KEY","")            # your private admin key
...
h2{font-family:'Orbitron',sans-serif;font-size:11px;margin-bottom:12px;letter-spacing:.08em;font-weight:700}
...
.s-prog{background:rgba(0,212,232,.1);color:var(--cyan)}
...
<section class="card full">
  <h2><span class="ico">⚡</span>TIKTOK BOOSTER</h2>
  ...
  <input type="url" id="tt-link"
    placeholder="https://tiktok.com/@username  or  https://tiktok.com/@user/video/ID">

  <label>Boost Type</label>
  <div class="type-row">
    <div class="ttype on" id="bt-likes" onclick="setType('likes')">❤️ &nbsp;LIKES</div>
    <div class="ttype" id="bt-followers" onclick="setType('followers')">👤 &nbsp;FOLLOWERS</div>
  </div>

  <label>Quantity</label>
  <div class="qty-row">
    <div class="qp on" onclick="setQty(this,100)">100</div>
    <div class="qp" onclick="setQty(this,500)">500</div>
    <div class="qp" onclick="setQty(this,1000)">1K</div>
    <div class="qp" onclick="setQty(this,5000)">5K</div>
    <div class="qp" onclick="setQty(this,10000)">10K</div>
    <input type="number" id="qty-c" placeholder="Custom…" min="1"
           style="max-width:110px;flex:none"
           oninput="document.querySelectorAll('.qp').forEach(b=>b.classList.remove('on'))">
  </div>

  <button class="btn" id="boost-btn" onclick="sendBoost()" style="margin-top:14px">
    ⚡ &nbsp;SEND BOOST
  </button>

  <!-- Order status bar -->
  <div class="order-bar" id="obar">
    <div>
      <div style="font-size:9px;color:var(--mut);font-family:'Orbitron',sans-serif;letter-spacing:.05em">ORDER ID</div>
      <div class="oid-val" id="oid-val">—</div>
    </div>
    <div class="osticker s-pend" id="osticker">PENDING</div>
    <button class="btn btn-sm alt" style="margin-left:auto" onclick="checkStatus()">↻ CHECK STATUS</button>
  </div>
...
<section class="card">
  ...
  <div style="font-size:9px;color:var(--mut);margin-bottom:8px">
    TikTok · YouTube · Instagram · Facebook · Twitter · Reddit · Twitch · 1000+ sites
  </div>
...
<section class="card">
  ...
  <div class="row" style="align-items:flex-end;margin-top:6px">
    <div><label>Lines per part</label>
      <input type="number" id="sp-n" value="1000" min="1"></div>
...
<!-- ═══ AI CHAT ════════════════════════════════════════════════════════════ -->
<section class="card full">
  <h2><span class="ico">🤖</span>AI CHAT — Claude</h2>
  <div class="cbox" id="cbox">
    <div class="bbl a">
      <div class="lbl">SHENRU AI</div>
      Wazzup! I'm your AI assistant. Ask me anything 🔥
    </div>
  </div>
  <div class="chat-row">
    <input type="text" id="cmsg" placeholder="Type a message…"
...
let   KEY  = localStorage.getItem('sh_key') || '';   // license key
let   bType = 'likes';
let   bQty  = 100;
let   lastOid = null;
...
function setLD(btn, on) {
  btn.disabled = on;
  on ? btn.classList.add('ld') : btn.classList.remove('ld');
}
function updStats() {
  $('st-sent').textContent  = stats.sent;
  $('st-likes').textContent = stats.likes.toLocaleString();
  $('st-foll').textContent  = stats.foll.toLocaleString();
}
...
async function tryKey() {
  ...
  if (d.ok) {
    KEY = k;
    localStorage.setItem('sh_key', k);
    $('gate').style.display = 'none';
    toast('Access granted 🔓', 'ok');
  } else {
    $('gerr').style.display = 'block';
    $('gkey').style.borderColor = 'var(--red)';
  }
}
...
const P = {
  ...
  init(){
    $('afiles').addEventListener('change', e => this.load(e.target.files));
    this.audio.addEventListener('ended', () => this.next());
  },
  load(files){
    const s = this.pl.length;
    Array.from(files).forEach(f =>
      this.pl.push({url:URL.createObjectURL(f), name:f.name.replace(/\.[^.]+$/,'')}));
    toast(files.length + ' track(s) loaded 🎵','ok');
    if (s===0) this.set(0);
  },
  set(i){
    if(!this.pl.length) return;
    this.cur = ((i%this.pl.length)+this.pl.length)%this.pl.length;
    const t = this.pl[this.cur];
    this.audio.src = t.url;
    $('ptitle').textContent = t.name;
    $('psub').textContent   = 'Track '+(this.cur+1)+' / '+this.pl.length;
    this.play();
  },
  play(){ this.audio.play().catch(()=>{}); $('pbtn').textContent='⏸'; $('disc').classList.add('spin'); },
  pause(){ this.audio.pause(); $('pbtn').textContent='▶'; $('disc').classList.remove('spin'); },
  toggle(){ this.audio.paused ? this.play() : this.pause(); },
  prev(){ this.set(this.cur-1); },
  next(){ this.set(this.cur+1); }
};

// ─── TIKTOK BOOSTER ───────────────────────────────────────────────────
function setType(t) {
  bType = t;
  ['likes','followers'].forEach(x =>
    $('bt-'+x).classList.toggle('on', x===t));
}
function setQty(el, q) {
  document.querySelectorAll('.qp').forEach(b => b.classList.remove('on'));
  el.classList.add('on');
  bQty = q;
  $('qty-c').value = '';
}
...
async function sendBoost() {
  ...
  const qty  = parseInt($('qty-c').value) || bQty;
  if (!link) return toast('Enter TikTok URL', 'warn');
  const btn = $('boost-btn');
  setLD(btn, true);
  show('boost-result','⏳ Sending boost…','warn');
  try {
    const d = await apiCall('/api/boost/order', {type: bType, link, quantity: qty});
    if (d.order) {
      lastOid = String(d.order);
      $('oid-val').textContent = lastOid;
      $('osticker').textContent = 'PENDING';
      $('osticker').className = 'osticker s-pend';
      $('obar').classList.add('show');
      stats.sent++;
      if (bType==='likes') stats.likes += qty;
      else stats.foll += qty;
      updStats();
      show('boost-result',
        '✅  BOOST ORDER PLACED!\n' +
        ...
        'Target    : ' + link + '\n' +
        'Status    : Pending → will start shortly\n\n' +
        'Hit CHECK STATUS to track delivery.', 'ok');
      toast('Boost sent! 🚀', 'ok');
    } else {
      show('boost-result', '❌ ' + (d.error || JSON.stringify(d)), 'err');
      toast(d.error || 'Order failed', '');
    }
  } catch(e) {
    show('boost-result', '❌ ' + e.message, 'err');
  }
  setLD(btn, false);
}

async function checkStatus() {
  if (!lastOid) return toast('No active order', 'warn');
  try {
    const d = await apiCall('/api/boost/status', {order_id: lastOid});
    const st = (d.status || '').toUpperCase();
    $('oid-val').textContent = lastOid;
    const sticker = $('osticker');
    sticker.textContent = st || 'UNKNOWN';
    sticker.className = 'osticker ' +
      (st==='COMPLETED'||st==='COMPLETE' ? 's-done' :
       st==='IN PROGRESS'||st==='INPROGRESS'||st==='PARTIAL' ? 's-prog' : 's-pend');
    show('boost-result', JSON.stringify(d, null, 2), st==='COMPLETED'?'ok':'');
    if (st==='COMPLETED'||st==='COMPLETE') toast('Order completed! ✅','ok');
  } catch(e) {
    show('boost-result', '❌ '+e.message, 'err');
  }
}
...
async function splitTxt(){
  const text=$('sp-txt').value, n=parseInt($('sp-n').value)||1000;
  if(!text.trim())return toast('Paste some text','warn');
  const d=await apiCall('/api/split',{text,lines:n});
  show('sp-result',d.output||d.error||'Done');
}
async function separateTxt(){
  const text=$('sep-txt').value, kw=$('sep-kw').value;
  if(!text.trim())return toast('Paste some text','warn');
  const d=await apiCall('/api/separate',{text,keywords:kw});
  show('sep-result',d.output||d.error||'Done');
}
async function shortenURL(){
  const url=$('s-url').value.trim();
  if(!url)return toast('Enter a URL','warn');
  const btn=$('s-btn');setLD(btn,true);
  const d=await apiCall('/api/shorten',{url});
  show('s-result',d.output||d.error,d.output?'ok':'err');
  setLD(btn,false);
}
...
async function sendChat(){
  const inp=$('cmsg'), msg=inp.value.trim();
  if(!msg)return; inp.value='';
  const box=$('cbox');
  const add=(html,cls)=>{
    const el=document.createElement('div');
    el.className='bbl '+cls; el.innerHTML=html;
    box.appendChild(el); box.scrollTop=box.scrollHeight; return el;
  };
  add(esc(msg),'u');
  chatHist.push({role:'user',content:msg});
  const typing=add('<div class="lbl">SHENRU AI</div><em style="color:var(--mut)">thinking…</em>','a');
  try{
    const d=await apiCall('/api/chat',{message:msg,history:chatHist.slice(0,-1)});
    if(d.error)throw new Error(d.error);
    typing.innerHTML='<div class="lbl">SHENRU AI</div>'+esc(d.response);
    chatHist.push({role:'assistant',content:d.response});
    if(chatHist.length>40)chatHist.splice(0,2);
  }catch(e){
    typing.innerHTML='<div class="lbl">SHENRU AI</div><span class="err">'+esc(e.message)+'</span>';
    chatHist.pop();
  }
...
async function aiHumanize(){
  const text=$('hum-in').value.trim();
  if(!text)return toast('Paste some text','warn');
  const btn=$('hum-btn');setLD(btn,true);
  show('hum-out','⏳ Humanizing…','warn');
  const d=await apiCall('/api/ai',{mode:'humanize',text});
  show('hum-out',d.output||d.error,d.output?'':'err');
  setLD(btn,false);
}
async function aiDetect(){
  const text=$('det-in').value.trim();
  if(!text)return toast('Paste some text','warn');
  const btn=$('det-btn');setLD(btn,true);
  show('det-out','⏳ Analyzing…','warn');
  const d=await apiCall('/api/ai',{mode:'detect',text});
  show('det-out',d.output||d.error,d.output?'':'err');
  setLD(btn,false);
}
...
def boost_order():
    if not PANEL_URL or not PANEL_KEY:
        return jsonify(error="Panel not configured — contact admin"), 503
    d       = request.get_json(force=True)
    btype   = d.get("type","likes")
    link    = d.get("link","").strip()
    qty     = max(10, min(int(d.get("quantity",100)), 100000))
    svc_id  = LIKES_SVC_ID if btype == "likes" else FOLLOWERS_SVC_ID
    if not svc_id:
        return jsonify(error=f"Service ID for {btype} not configured — run /api/admin/discover"), 503
    if not link:
        return jsonify(error="No TikTok URL provided")
    try:
        result = smm(PANEL_URL, PANEL_KEY, action="add", service=svc_id, link=link, quantity=qty)
...
def boost_status():
    ...
    oid = d.get("order_id")
    if not oid:
        return jsonify(error="No order ID")
    try:
        return jsonify(smm(PANEL_URL, PANEL_KEY, action="status", order=oid))
    except Exception as e:
        return jsonify(error=str(e)), 500

# ── ADMIN: discover service IDs ─────────────────────────────────────────────
@app.get("/api/admin/discover")
def admin_discover():
    if not ADMIN_KEY:
        return jsonify(error="Set ADMIN_KEY env var to use this endpoint"), 403
    if request.args.get("key","") != ADMIN_KEY:
        return jsonify(error="Unauthorized"), 403
    if not PANEL_URL or not PANEL_KEY:
        return jsonify(error="Set PANEL_URL and PANEL_KEY env vars first"), 503
    try:
        raw  = smm(PANEL_URL, PANEL_KEY, action="services")
        svcs = raw if isinstance(raw, list) else []
        tt   = [s for s in svcs
                if "tiktok" in s.get("name","").lower()
                or "tiktok" in s.get("category","").lower()
                or "tt" in s.get("category","").lower()]
        auto_likes = next((s for s in tt if "like" in s.get("name","").lower()), None)
        auto_foll  = next((s for s in tt if "follow" in s.get("name","").lower()), None)
        return jsonify(
            hint="Copy the IDs below into your env vars: LIKES_SERVICE_ID and FOLLOWERS_SERVICE_ID",
            auto_detected_likes    = auto_likes,
            auto_detected_followers= auto_foll,
            current_likes_id       = LIKES_SVC_ID or "NOT SET",
            current_followers_id   = FOLLOWERS_SVC_ID or "NOT SET",
            total_tiktok_services  = len(tt),
            all_tiktok_services    = tt[:60]
        )
    except Exception as e:
        return jsonify(error=str(e)), 500
    ...
    try:
        ...
        if not os.path.exists(path):
            for p in [f"{tmp}.mp4",f"{tmp}.webm",f"{tmp}.mkv"]:
                if os.path.exists(p): path=p; ext=p.split(".")[-1]; break
        @after_this_request
        def _clean(r):
            try: os.remove(path)
            except: pass
...
@require_key
def split():
    d=request.get_json(force=True); text=str(d.get("text",""))
    n=max(1,min(int(d.get("lines",1000)),100000))
    if not text: return jsonify(error="No text")
    lines=text.splitlines(); chunks=[lines[i:i+n] for i in range(0,len(lines),n)]
...
def separate():
    ...
    if not text: return jsonify(error="No text")
    buckets={k:[] for k in kws}; other=[]
    for line in text.splitlines():
        low=line.lower(); hit=next((k for k in kws if k in low),None)
...
@require_key
def shorten():
    d=request.get_json(force=True); url=str(d.get("url","")).strip()
    ...
    try:
        ...
        with urllib.request.urlopen(req,timeout=8) as r: short=r.read().decode().strip()
...
@require_key
def ai():
    if not ANTHROPIC_KEY: return jsonify(output="⚠️ Set ANTHROPIC_API_KEY to enable AI.")
    d=request.get_json(force=True); mode=d.get("mode",""); text=str(d.get("text","")).strip()
    if not text: return jsonify(error="No text")
    try:
        prompt=("Rewrite this AI-generated text to sound completely human. "
                "Strip AI patterns. Return ONLY rewritten text:\n\n"+text
                if mode=="humanize" else
                "Analyze if this text is AI-generated or human. "
                "Give a score 0-100%, verdict, and 3 key indicators:\n\n"+text)
        return jsonify(output=claude([{"role":"user","content":prompt}],max_tokens=2000))
    except Exception as e: return jsonify(error=str(e))
