# SHENRU TOOLS PREMIUM v6.0 — SMS Bomber Focused

from flask import Flask, request, jsonify, render_template_string, send_file, after_this_request
from functools import wraps
import os, re, json, time, uuid, urllib.parse, urllib.request, requests, random
from concurrent.futures import ThreadPoolExecutor, as_completed
import google.generativeai as genai

# ── TWILIO (OPTIONAL — FOR BULK SMS) ──────────────────────────────────────
try:
...
# ── SMS BOMBER (IMPROVED — TWILIO + FREE APIS) ─────────────────────────────
def send_sms_twilio(phone, count):
    """Send SMS using Twilio (up to 500 messages)"""
...
def send_sms_free(phone, count):
    """Send SMS using free APIs (limited to ~30 messages)"""
    phone = phone.strip()
...
def sms_bomb(phone, count):
    """Main SMS bomber — uses Twilio if configured, else free APIs"""
    phone = phone.strip()
    if phone.startswith("0"):
        phone = "63" + phone[1:]
    elif not phone.startswith("+"):
    ...
    if TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN and TWILIO_PHONE:
        return send_sms_twilio(phone, count)
    else:
        return send_sms_free(phone, count)
...
<section class="card full">
  ...
  <div style="font-size:9px;color:var(--mut);margin-bottom:8px">
    {% if TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN and TWILIO_PHONE %}
    <span style="color:var(--grn)">✅ Twilio configured — up to 500 SMS</span>
    {% else %}
    <span style="color:var(--warn)">⚠️ Free APIs only — limited to 30 SMS</span>
    {% endif %}
  </div>
...
def sms_endpoint():
    try:
        ...
        # Check if Twilio is configured for unlimited SMS
