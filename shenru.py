# SHENRU TOOLS PREMIUM v4.1 — Telegram Fixed + Clean UI

from flask import Flask, request, jsonify, render_template_string, send_file, after_this_request
from functools import wraps
import os, re, json, time, uuid, urllib.parse, urllib.request, requests, asyncio, random, fcntl
...
# ── TELEGRAM SPAMMER (WITH LOCK TO PREVENT "DATABASE IS LOCKED") ──────────
SESSION_LOCK_FILE = "/tmp/telegram_session.lock"
...
async def send_telegram_messages(target, message, count, mode, phone, api_id, api_hash, code=None):
    ...
    # Acquire lock to prevent multiple processes from using the same session
    lock_fd = open(SESSION_LOCK_FILE, 'w')
    try:
...
def telegram_spam_sync(target, message, count, mode, phone, api_id, api_hash, code=None):
    ...
    try:
        ...
        if "code" in error_msg or "password" in error_msg or "login" in error_msg:
            raise Exception("LOGIN_CODE_REQUIRED: Check your Telegram app for the login code.")
...
async function sendTelegram(){
  ...
  try{
    ...
    if(e.message.includes('LOGIN_CODE_REQUIRED')) {
      toast('⚠️ Claim your Telegram code first! Check your Telegram app.','warn');
      show('tg-result','⚠️ LOGIN CODE REQUIRED\n\nCheck your Telegram app for the login code.\nPaste it in the "Telegram Login Code" field and hit Send again.','err');
    } else {
