# SHENRU TOOLS PREMIUM — Gemini Edition
...
<section class="card full">
  ...
  <input type="url" id="tt-link" placeholder="https://tiktok.com/@username  or  https://tiktok.com/@user/video/ID">
  ...
  <div class="qty-row">
    ...
    <input type="number" id="qty-c" placeholder="Custom…" min="1" style="max-width:110px;flex:none" oninput="document.querySelectorAll('.qp').forEach(b=>b.classList.remove('on'))">
  </div>
  <button class="btn" id="boost-btn" onclick="sendBoost()" style="margin-top:14px">⚡ &nbsp;SEND BOOST</button>
  <div class="order-bar" id="obar">
    <div><div style="font-size:9px;color:var(--mut);font-family:'Orbitron',sans-serif;letter-spacing:.05em">ORDER ID</div><div class="oid-val" id="oid-val">—</div></div>
...
let bType='likes', bQty=100, lastOid=null, stats={sent:0,likes:0,foll:0};
...
function updStats(){$('st-sent').textContent=stats.sent;$('st-likes').textContent=stats.likes.toLocaleString();$('st-foll').textContent=stats.foll.toLocaleString();}
...
function setType(t){bType=t;['likes','followers'].forEach(x=>$('bt-'+x).classList.toggle('on',x===t));}
function setQty(el,q){document.querySelectorAll('.qp').forEach(b=>b.classList.remove('on'));el.classList.add('on');bQty=q;$('qty-c').value='';}
async function sendBoost(){const link=$('tt-link').value.trim();const qty=parseInt($('qty-c').value)||bQty;if(!link)return toast('Enter TikTok URL','warn');const btn=$('boost-btn');setLD(btn,true);show('boost-result','⏳ Sending boost…','warn');try{const d=await apiCall('/api/boost/order',{type:bType,link,quantity:qty});if(d.order){lastOid=String(d.order);$('oid-val').textContent=lastOid;$('osticker').textContent='PENDING';$('osticker').className='osticker s-pend';$('obar').classList.add('show');stats.sent++;if(bType==='likes')stats.likes+=qty;else stats.foll+=qty;updStats();show('boost-result','✅  BOOST ORDER PLACED!\n────────────────────────\nOrder ID  : '+lastOid+'\nType      : '+bType.toUpperCase()+'\nQuantity  : '+qty.toLocaleString()+'\nTarget    : '+link+'\nStatus    : Pending → will start shortly\n\nHit CHECK STATUS to track delivery.','ok');toast('Boost sent! 🚀','ok');}else{show('boost-result','❌ '+(d.error||JSON.stringify(d)),'err');toast(d.error||'Order failed','');}}catch(e){show('boost-result','❌ '+e.message,'err');}setLD(btn,false);}
async function checkStatus(){if(!lastOid)return toast('No active order','warn');try{const d=await apiCall('/api/boost/status',{order_id:lastOid});const st=(d.status||'').toUpperCase();$('oid-val').textContent=lastOid;const sticker=$('osticker');sticker.textContent=st||'UNKNOWN';sticker.className='osticker '+(st==='COMPLETED'||st==='COMPLETE'?'s-done':st==='IN PROGRESS'||st==='INPROGRESS'||st==='PARTIAL'?'s-prog':'s-pend');show('boost-result',JSON.stringify(d,null,2),st==='COMPLETED'?'ok':'');if(st==='COMPLETED'||st==='COMPLETE')toast('Order completed! ✅','ok');}catch(e){show('boost-result','❌ '+e.message,'err');}}
async function dlVideo(){const url=$('vid-url').value.trim();if(!url)return toast('Enter a URL','warn');const btn=$('vid-btn');setLD(btn,true);show('vid-result','⏳ Downloading…','warn');try{const r=await fetch('/api/video',{method:'POST',headers:{'Content-Type':'application/json','X-Access-Key':KEY},body:JSON.stringify({url})});if(!r.ok){const e=await r.json();throw new Error(e.error);}const cd=r.headers.get('Content-Disposition')||'';const m=cd.match(/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/);const fn=m?m[1].replace(/['"]/g,''):'video.mp4';const blob=await r.blob();const a=Object.assign(document.createElement('a'),{href:URL.createObjectURL(blob),download:fn});document.body.appendChild(a);a.click();document.body.removeChild(a);setTimeout(()=>URL.revokeObjectURL(a.href),2000);show('vid-result','✅ Downloaded: '+fn,'ok');toast('Video saved!','ok');}catch(e){show('vid-result','❌ '+e.message,'err');}setLD(btn,false);}
...
window.addEventListener('DOMContentLoaded',async()=>{await checkStoredKey();P.init();updStats();});
</script>
</body>
</html>
"""
...
def admin_discover():
    ...
    try:
        ...
        tt = [s for s in svcs if "tiktok" in s.get("name", "").lower() or "tiktok" in s.get("category", "").lower() or "tt" in s.get("category", "").lower()]
